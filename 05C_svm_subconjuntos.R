...
# Responsabilidade:
# 1) fase exploratoria barata para SVM radial em subconjuntos candidatos;
...
for (idx_sub in seq_along(subconjuntos)) {
  ...
  resultados_exploratorios[[idx_sub]] <- extrair_melhor_resultado_caret(
    modelo_svm,
    metadata = list(
...
# ------------------------------------------------------------------------------
tabela_svm <- dplyr::bind_rows(resultados_confirmacao) %>%
  dplyr::select(
    Subconjunto, TopN, Modelo,
    ROC, Sens, Spec, Precision, F1, GMean,
    dplyr::everything()
...
if (file.exists(caminho_objeto_saida("confirmacao", "tabela_benchmark_glm_rf_xgb_sem_balanceamento.rds", subpastas = "benchmark")) ||
...
if (file.exists(caminho_objeto_saida("exploratorio", "tabela_benchmark_glm_rf_xgb_sem_balanceamento_exploratorio.rds", subpastas = "benchmark")) ||
...
grafico_benchmark_tecnico <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = "Benchmark tecnico confirmado - ate SVM radial",
    x = "ROC",
    y = NULL
  ) +
...
salvar_rds_saida(
  tabela_exploratoria,
  "exploratorio",
  "tabela_svm_subconjuntos_sem_balanceamento_exploratorio.rds",
  subpastas = "benchmark"
)
salvar_csv_saida(
  tabela_exploratoria,
  "exploratorio",
  "tabela_svm_subconjuntos_sem_balanceamento_exploratorio.csv",
  subpastas = "benchmark"
)
...
salvar_csv_saida(
  tabela_svm,
  "confirmacao",
  "tabela_svm_subconjuntos_sem_balanceamento.csv",
  subpastas = "benchmark"
)
...
salvar_csv_saida(
  tabela_benchmark_exploratoria,
  "exploratorio",
  "tabela_benchmark_glm_rf_xgb_svm_sem_balanceamento_exploratorio.csv",
  subpastas = "benchmark"
)
...
salvar_csv_saida(
  tabela_benchmark_completa,
  "confirmacao",
  "tabela_benchmark_glm_rf_xgb_svm_sem_balanceamento.csv",
  subpastas = "benchmark"
)
...
salvar_figura_saida(
  ...
  fase = "confirmacao",
  arquivo = "roc_benchmark_glm_rf_xgb_svm_tecnico.png",
  subpastas = "benchmark",
